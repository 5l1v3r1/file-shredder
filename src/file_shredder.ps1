...
if ($args.Count -ne 1) {
	...
	try {
		...
		$file = Get-Item $args[0] -ErrorAction SilentlyContinue;
		if ($file -eq $null) {
			...
			$sectors = [Math]::Ceiling($file.Length / 2048.0);
			...
			$file.Delete();
			Write-Host "File was shredded successfully.";
		}
	} catch {
